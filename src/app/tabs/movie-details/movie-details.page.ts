...
export class MovieDetailsPage implements OnInit {
  ...
  // Function are halt temporarily
  openMovie() {
    const id = this.route.snapshot.paramMap.get('id')!;
    this.contentService.getMoviePlayer(id).subscribe((data) => {
      console.log('Movie Found!', data);
      
      // Assuming data.results is an array and you want to open the first video
      if (data.results && data.results.length > 0) {
        const firstVideoUrl = data.results[0].videoUrl; // Replace 'videoUrl' with the actual property in your data
